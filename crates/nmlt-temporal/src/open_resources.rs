...
mod hidden_consume_c1 {
    ...
    fn hidden_unconnected_ping_consume_fails_resource_but_observation_passes() {
        ...
        let observation =
            RefinementChecker::check(concrete.graph(), abstract_system.graph(), &spec);
        assert!(
    ...
    fn mapped_ping_consume_does_not_match_abstract_inert() {
        ...
        let concrete_resources =
            SystemResourceProfile::new(["token"], [("ping", ping_consume_profile())]).unwrap();
        let abstract_resources =
            SystemResourceProfile::new(["token"], [("ping", ping_require_no_consume_profile())])
                .unwrap();
...
mod hidden_grade_c1 {
    ...
    fn ping_cost_profile() -> ActionResourceProfile {
        ActionResourceProfile::new(
            Vec::<&str>::new(),
            Vec::<&str>::new(),
            Vec::<&str>::new(),
            Vec::<&str>::new(),
            Grade::checked(1, 0, 0, UncertaintyCertificate::Certain).unwrap(),
            Vec::<&str>::new(),
            Vec::<&str>::new(),
        )
        .unwrap()
    ...
    /// Hidden ping still observation-refines the step-free abstract sender,
    /// but a non-epsilon cost makes the resource checker reject. Isolates
    /// I-GRADE: no consume/authority mismatch, only a positive grade on a
    /// hidden action. Connection is orthogonal.
    #[test]
    fn hidden_unconnected_ping_grade_fails_resource_but_observation_passes() {
        let concrete = ping_output_system(true);
        let abstract_system = empty_abstract_system();
        let concrete_resources =
            SystemResourceProfile::new(Vec::<&str>::new(), [("ping", ping_cost_profile())])
                .unwrap();
        let abstract_resources = SystemResourceProfile::new(
            Vec::<&str>::new(),
            Vec::<(&str, ActionResourceProfile)>::new(),
        )
        .unwrap();
        let spec = observation_spec(true);
...
mod hidden_rely_c1 {
    ...
    fn ping_extra_rely_profile() -> ActionResourceProfile {
        ActionResourceProfile::new(
            Vec::<&str>::new(),
            Vec::<&str>::new(),
            Vec::<&str>::new(),
    ...
    /// Hidden ping still observation-refines the step-free abstract sender,
    /// but an extra concrete rely fact makes the resource checker reject.
    /// Isolates I-RELY: empty caps/grades, only a widened assumption on a
    /// hidden action. Connection is orthogonal.
    #[test]
    fn hidden_unconnected_ping_rely_fails_resource_but_observation_passes() {
        let concrete = ping_output_system(true);
        let abstract_system = empty_abstract_system();
        let concrete_resources =
            SystemResourceProfile::new(Vec::<&str>::new(), [("ping", ping_extra_rely_profile())])
                .unwrap();
        let abstract_resources = SystemResourceProfile::new(
            Vec::<&str>::new(),
            Vec::<(&str, ActionResourceProfile)>::new(),
...
mod hidden_inert_c1 {
    ...
    use super::{ActionResourceProfile, ResourceRefinementChecker, SystemResourceProfile};
    use crate::graph::{FiniteGraph, ModelState, Transition, Value};
    use crate::observation::{ActionHiding, ObservationMap};
    use crate::open::{ActionSignature, Interface, OpenSystem};
    use crate::open_contract::{FiniteContract, PayloadPredicate, PayloadType};
    ...
    /// Hidden ping still observation-refines the step-free abstract sender,
    /// and an inert (no consume / epsilon grade / no rely) profile makes the
    /// resource checker accept. Dual of the three C1 negatives: matching
    /// stutter profiles lift. Connection is orthogonal; not a compose lift.
    #[test]
    fn hidden_unconnected_ping_inert_passes_observation_and_resource() {
        let concrete = ping_output_system(true);
        let abstract_system = empty_abstract_system();
        let concrete_resources = SystemResourceProfile::new(
            Vec::<&str>::new(),
            [("ping", ActionResourceProfile::inert())],
        )
        .unwrap();
        let abstract_resources = SystemResourceProfile::new(
            Vec::<&str>::new(),
        ...
        assert!(
            resources.accepted,
            "inert hidden ping must resource-refine: {:#?}",
            resources.issues
        );
    }

    /// Same matching inert profiles with ping mapped to the ping *name*
    /// (Lean `mapPing` / `SenderLabel` singleton). Observation refinement
    /// would *not* pass this visible mapping against a step-free abstract
    /// sender; this is only the resource side of
    /// `hiddenPing_inert_resourceRefinement`.
    #[test]
    fn mapped_ping_inert_matches_abstract_inert() {
        let concrete = ping_output_system(true);
        let abstract_system = ping_output_system(false);
        let concrete_resources = SystemResourceProfile::new(
            Vec::<&str>::new(),
        ...
        assert!(
            resources.accepted,
            "matching inert profiles must resource-refine along mapPing: {:#?}",
            resources.issues
        );
    }
}
