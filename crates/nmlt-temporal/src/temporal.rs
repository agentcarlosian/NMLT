...
mod tests {
    ...
    fn hidden_ping_loop_eventually_fails_under_weak_ping_fairness() {
        ...
        .unwrap();
        let checker = TemporalChecker::new(&graph, FairnessSet::new(vec![Fairness::weak("ping")]));
    ...
    fn empty_wiring_sender_receiver_eventually_peer_bit_fails() {
        ...
        assert!(
            !outcome.holds(),
            "ping-only fairness must not guarantee eventual progress of the peer bit"
        );
        let CheckOutcome::Violated { witness, .. } = outcome else {
        ...
        assert!(
            witness
                .loop_states
                .iter()
                .all(|&state| checker.graph().state(state).get("bit") == Some(&Value::Bool(false))),
