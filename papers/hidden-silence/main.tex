...
This manuscript is a working research draft.  The Lean declarations and the
repository's current claim ceiling are authoritative when prose here is
incomplete.  In particular, the current development does not establish
minimality or logical necessity of every product-formation field.
...
single static behavior object and prove that it lifts through the current
binary composition when the wiring is preserved and both products are admitted
by the formation judgment.  Product formation
...
The current static product closes synchronized actions and does not yet preserve
a general open product interface.  A separate dynamic resource-world layer
tracks authority movement for supplied product steps; the artifact certificate
does not establish step existence or reachability.
No path, trace, or reachability adequacy theorem has yet been proved for the
resource-weak-refinement relation.  The result establishes closure of the
defined one-step relation under the current product.
...
A product admitted by the current formation judgment establishes:
\begin{enumerate}
...
The formation judgment is also the language boundary for several invariants.
The current lifting proof does not consume every field of that judgment, so no
minimality or per-field necessity result is claimed.
...
The explicit state refinement is identity.  Rust exploration of the concrete
visible synchronization moves authority from the sender to the receiver,
changes the receiver's observed bit, and reports composed work grade three.
Lean decodes the
...
The dynamic theorem
\texttt{NMLT.Artifact.SemanticClosure.Certificate.liftedStep} maps any supplied
concrete dynamic product step to a matching abstract one.  It is conditional on
that step; the present artifact checker does not derive an initial step or prove
reachability.  The static pair-state behavior and the dynamic resource-world
state are related operationally by design but do not yet have a proved
correspondence theorem.

\section{Negative controls}

Permanent controls independently violate each of the following formation or
refinement obligations:
\begin{center}
\begin{tabular}{ll}
\toprule
Violated obligation & Rejected configuration\\
\midrule
hidden-boundary isolation & a hidden action remains connected\\
whole-wiring preservation & the two connection relations disagree\\
capability partition & two ownership predicates contain the same authority\\
...
The controls establish distinct rejected boundaries.  They do not prove that
every field is logically necessary for every possible congruence statement.
