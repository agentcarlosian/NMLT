...
Observation lift through compose is not resource lift through compose. T5 already lifts the empty-wiring hidden tau as an observation (`emptyWiring_productRefinement`). If that tau is decorated with a `token` consume the abstract stutter does not have, the *product* still observation-refines, but product resource refinement fails (`emptyWiring_hiddenConsume_breaks_productResourceRefinement`). If both sides use the matching inert stutter, the product resource profile does lift (`emptyWiring_inert_productResourceRefinement`) — the first compose resource homomorphism on this small model. That is not I-FAIR, not hidden-connected ping, not OpenComposition, and not the full RFC 0008 CONDITIONAL-CONGRUENCE story (sync transfer, grades through sync, fairness through sync remain open).
