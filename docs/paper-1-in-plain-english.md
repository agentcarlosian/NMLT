...
Visible sync is a different independence. T5 already lifts VisibleSync as an observation (`visibleSync_productRefinement`: visible ping wired to Lean `receive`). If that ping transfers a `token` the receiver does not receive, `SynchronizationCompatible` fails (`visibleSync_unmatchedTransfer_not_synchronizationCompatible`) even though the product still observation-refines *and* product resource refinement still holds — because the sync action zeros transfer/receive. Matching transfer is the extra I-CAP premise (`visibleSync_matchedTransfer_synchronizationCompatible`); T5 does not imply it. Shared ownership of the same `token` independently fails `CapabilityPartition` (`visibleSync_sharedOwnership_not_capabilityPartition`). Grades through sync and fairness through sync remain open.
