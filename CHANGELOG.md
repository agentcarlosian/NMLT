...
- C1 Case 7 sync-grade I-GRADE on Paper 1 VisibleSync (small LTS): named Lean `visibleSync_hiddenGrade_breaks_productResourceRefinement` (`[propext]` only) — T5a VisibleSync product `WeakRefines` still holds (visible ping wired to Lean `receiver`), but concrete ping carries `HiddenGrade.pingCost` against receiver/abstract epsilon so product `ResourceRefinement.grade` fails at `.sync ping receive` (`parallelAction` = `Grades.parallel`; concrete parallel cost not ≤ abstract parallel zero). T5 still holds (observation only). Named alias `visibleSync_matchedTransfer_syncGrade` (`[propext]` only) — matching epsilon/`zero` grades already lift via `liftParallelResources` on matched-transfer inert profiles; the `.sync` grade inequality holds. Ceiling: T5 ⇏ I-GRADE through sync. Not I-FAIR, not unmatched transfer (those lemmas kept). Full CONDITIONAL-CONGRUENCE still open (I-FAIR through sync). Lean `receiver` is not the OpenSystem-receptive dual.

- C1 Case 7 sync-transfer I-CAP on Paper 1 VisibleSync (small LTS): named Lean `visibleSync_unmatchedTransfer_not_synchronizationCompatible` (`[propext]` only) — T5a VisibleSync product `WeakRefines` still holds (visible ping wired to Lean `receiver`), but ping transfers `token` unmatched by receive so `SynchronizationCompatible` fails, while product `ResourceRefinement` still holds via `liftParallelResources` because `parallelAction` zeros transfer/receive (T5 + product ResourceRefinement ⇏ I-CAP transfer on sync). Named Lean `visibleSync_matchedTransfer_synchronizationCompatible` (`[propext]` only) — matching ping transfer / receive receive is the extra I-CAP premise (instantiates `synchronized_transfer_exact` / `synchronized_rely_discharged`); product `ResourceRefinement` lifts when profiles otherwise match. Optional `visibleSync_sharedOwnership_not_capabilityPartition` (both own `token` ⇒ ¬ `CapabilityPartition`; T5 still holds). Ceiling: T5 ⇏ I-CAP transfer on sync. Not hidden ping, not empty-wiring consume, not I-FAIR, not grades through *sync*. Full CONDITIONAL-CONGRUENCE still open (I-FAIR through sync; grades through *sync* now checked). Lean `receiver` is not the OpenSystem-receptive dual.

- C1 Case 7 product resource-independence pack (small LTS): named Lean `emptyWiring_hiddenGrade_breaks_productResourceRefinement` (`[propext]` only) — T5b EmptyWiringTau product `WeakRefines` still holds, but a hidden left tau with a positive cost atom unmatched by abstract epsilon/`zero` falsifies product `ResourceRefinement.grade` (T5 ⇏ I-GRADE through compose; HiddenGrade pattern on `smallProductSystemResources`; not I-FAIR, not sync). Named Lean `emptyWiring_hiddenRely_breaks_productResourceRefinement` (`[propext]` only) — extra hidden-tau `ready` Fact unmatched by abstract stutter falsifies product `ResourceRefinement.rely` (T5 ⇏ I-RELY through compose; HiddenRely pattern). Pack frozen with existing `emptyWiring_inert_productResourceRefinement` (positive) and `emptyWiring_hiddenConsume_breaks_productResourceRefinement` (I-CAP through compose negative). Full CONDITIONAL-CONGRUENCE still open (I-FAIR through sync; VisibleSync I-CAP transfer and grades through *sync* now checked).

- C1 Case 7 first slice (small LTS): named Lean `liftParallelResources` (`[propext]` only) lifts left `ResourceRefinement` through `NMLT.Core.Transition.parallel` / `compositeMapOf` with a fixed peer (`ParallelLabel` = left | right | sync). Instantiation `emptyWiring_inert_productResourceRefinement` (T5b EmptyWiringTau + matching inert profiles; first compose resource homomorphism on the small model). Negative `emptyWiring_hiddenConsume_breaks_productResourceRefinement`: same empty-wiring systems still product-WeakRefine, extra hidden-tau consume falsifies product `ResourceRefinement` (T5 ⇏ I-CAP through compose). Ceiling: empty wiring, matching profiles, default `compositeMapOf`. Not I-FAIR, not hidden-connected ping, not OpenComposition T4, not full CONDITIONAL-CONGRUENCE (sync transfer I-CAP, grades through sync, I-FAIR through sync remain open).

- First C1 I-FAIR *negative*: named Lean `hiddenPing_divergence_not_discharged_by_weakRefines` (prefer axiom-free). Observational `WeakRefines` (T1) permits hidden divergence: ConcreteSender takes hidden `ping` at every `Nat` index (`Nat → state` path) and stays at the unique abstract state. Finite observation-trace inclusion still holds on every finite prefix, so T5 does not discharge I-FAIR / RFC 0007 R-DIVERGENCE. Not a liveness theorem; not WF/SF transport; not an I-FAIR lift through sync. Case 7 compose resource homomorphism remains open. Executable `nmlt-temporal` control: a one-state ping loop fails `eventually` of a never-reached observation even under `Fairness::weak("ping")`; empty-wiring sender||receiver fails `eventually` peer bit true under ping-only fairness.

- Residual C1 *positive* sharpness: named Lean `hiddenPing_inert_resourceRefinement` (`[propext]` only). If the hidden-ping resource profile matches the abstract stutter (no extra consume, epsilon grade, no extra rely), `ResourceRefinement` holds along `mapPing` while T1 WeakRefines still holds. The three C1 negatives (I-CAP consume, I-GRADE cost, I-RELY extra ready) are independent of this matching case. Case 7 of RFC 0008 remains open for the *compose* lift (not a product resource homomorphism; not I-FAIR). This resource-independence pack is frozen. Executable `nmlt-temporal` control: observation and resource checkers both accept an inert hidden ping.

- Sibling residual C1 *negative*: named Lean `hiddenPing_rely_breaks_resourceRefinement` (`[propext]` only). Observational `WeakRefines` (T1 / T5 small model) does not imply I-RELY: hidden ping still refines the step-free abstract sender, but an extra `ready` Fact unmatched by the abstract stutter profile falsifies `ResourceRefinement.rely` (refinement must not widen assumptions). Case 7 of RFC 0008 remains open (no positive I-RELY lift; not I-FAIR; not a rely discharge through compose). Executable `nmlt-temporal` control: hidden ping with extra concrete rely is `HiddenActionHasResources` while observation refinement still accepts.
